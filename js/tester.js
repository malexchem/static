...
// Also update the loadCurrentSalaryCycle to ensure it properly refreshes
async function loadCurrentSalaryCycle() {
    try {
        console.log('Loading current salary cycle...');
        ...
        if (data.success) {
            console.log('Current salary cycle loaded:', data.data);
            currentSalaryCycle = data.data;
            showCurrentCycleSection();
            loadSalaryCycleEmployees();
        } else {
            console.log('No active salary cycle available');
            hideCurrentCycleSection();
        }
    } catch (err) {
        console.error('Failed to load current salary cycle', err);
...
function setupEventListeners() {
    ...
    const saveSalaryDetails = document.getElementById('saveSalaryDetails');
    ...
    if (saveSalaryDetails) saveSalaryDetails.addEventListener('click', saveSalaryDetails);
...
// Update the loadSalaryCycleEmployees to handle data refresh better
function loadSalaryCycleEmployees() {
    if (!currentSalaryCycle || !salaryCycleTableBody) {
        console.log('No current salary cycle or table body found');
        return;
    }
    
    console.log('Loading salary cycle employees:', currentSalaryCycle.employees);
    ...
    currentSalaryCycle.employees.forEach((entry, index) => {
        const employee = entry.employee || {};
        const total = (entry.basicSalary || 0) + (entry.commission || 0) + (entry.allowances || 0) - (entry.deductions || 0);
        
        console.log(`Employee ${index}:`, employee.firstName, 'Total:', total);
    ...
    attachSalaryCycleEventListeners();
    
    updateSelectAllCheckbox();
}

// Separate function to attach event listeners to salary cycle elements
function attachSalaryCycleEventListeners() {
    // Add event listeners to checkboxes
    document.querySelectorAll('.employee-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            const id = e.target.getAttribute('data-id');
            console.log('Checkbox changed for employee:', id);
            // Note: In a real implementation, you might want to update the backend
            // when an employee is selected/deselected
        });
    });
...
// Add a refresh function to manually refresh the current cycle
async function refreshCurrentSalaryCycle() {
    console.log('Refreshing current salary cycle...');
    await loadCurrentSalaryCycle();
}
...
async function saveSalaryDetails() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // IMPORTANT: Update the current salary cycle with the returned data
            currentSalaryCycle = data.data;
